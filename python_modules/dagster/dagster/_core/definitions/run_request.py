...
class RunRequest(IHaveNew, LegacyNamedTupleMixin):
    ...
    Args:
        run_key (Optional[str]): A string key to identify this launched run. Semantics differ for
            sensors versus schedules.
            - For sensors, ensures that only one run is created per run key across all sensor
              evaluations for the current sensor. Passing in a ``None`` value means that a run will
