...
def test_scheduled_execution_time_iso_naive_datetime_uses_local_time():
    # Naive datetimes are treated as local time by astimezone(), which is
    # OS-locale-dependent. We can't assert a specific UTC value, but we can
    # assert the result is a valid UTC ISO string and that two naive datetimes
    # that differ by a known offset produce results that differ by that same offset.
...
def test_tags_for_scheduled_execution_time_no_run_key():
    ...
    assert unique_key == f"schedule:my_schedule:{expected_iso}"
...
def test_tags_for_scheduled_execution_time_with_run_key():
    ...
    assert unique_key == f"schedule:my_schedule:partition_A:{expected_iso}"
    assert runs_filter == RunsFilter(tags={SCHEDULED_EXECUTION_TIME_TAG: expected_iso})
