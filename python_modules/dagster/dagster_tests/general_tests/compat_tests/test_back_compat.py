...
def test_re_add_run_tags_run_id_idx():
    src_dir = dg.file_relative_path(__file__, "snapshot_1_13_3_re_add_run_tags_run_id_idx/sqlite")

    with copy_directory(src_dir) as test_dir:
        db_path = os.path.join(test_dir, "history", "runs.db")

        # Before migration
        assert get_current_alembic_version(db_path) == "29b539ebc72a"
        assert "run_tags" in get_sqlite3_tables(db_path)
        assert "idx_run_tags" in get_sqlite3_indexes(db_path, "run_tags")
        assert "idx_run_tags_run_id" not in get_sqlite3_indexes(db_path, "run_tags")
        ...
        # After downgrade (same as **after** migration, since 051_re_add_run_tags_run_id_idx is
        # _supposed_ to be a no-op)
        instance._run_storage._alembic_downgrade(rev="29b539ebc72a")  # pyright: ignore[reportAttributeAccessIssue]
        assert get_current_alembic_version(db_path) == "29b539ebc72a"
        assert "run_tags" in get_sqlite3_tables(db_path)
        assert "idx_run_tags" not in get_sqlite3_indexes(db_path, "run_tags")
        assert "idx_run_tags_run_id" in get_sqlite3_indexes(db_path, "run_tags")
